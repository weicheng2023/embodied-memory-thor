#!/usr/bin/env python3
"""Authorize and execute one complete fresh privacy-safe formal-v5 matrix."""
...
DEFAULT_AUTHORIZATION = (
    PROJECT_ROOT / "configs" / "phase5_real_formal_execution_v5.json"
)
AUTHORIZATION_VERSION = "phase5-real-thor-formal-execution-authorization-v5"
BASE_PROTOCOL_VERSION = "phase5-real-thor-pilot-v5"
READINESS_EVIDENCE_VERSION = "phase5-real-thor-formal-readiness-v5-result-v1"
AUTHORIZATION_EXECUTOR = "scripts/run_phase5_real_formal_execution_v5.py"


def _implementation() -> Any:
    path = PROJECT_ROOT / "scripts" / "run_phase5_real_formal_pilot_v2.py"
    spec = importlib.util.spec_from_file_location("phase5_real_formal_v5_frozen", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load frozen formal-v5 executor")
...
def _project_file(relative: str) -> Path:
    ...
    try:
        ...
        raise ValueError("formal-v5 authorization source is outside the project") from exc
    return path
...
def load_authorized_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("formal-v5 authorization must be a mapping")
    allowed = {
    ...
    if set(map(str, raw)) != allowed:
        raise ValueError("formal-v5 authorization has unexpected fields")
    if raw.get("authorization_version") != AUTHORIZATION_VERSION:
        raise ValueError("formal-v5 authorization version mismatch")
    if raw.get("formal_execution_authorized") is not True:
        raise ValueError("formal-v5 execution is not explicitly authorized")
    if raw.get("matrix_contract_override_allowed") is not False:
        raise ValueError("formal-v5 authorization cannot override the matrix")
    if raw.get("authorization_executor") != AUTHORIZATION_EXECUTOR:
        raise ValueError("formal-v5 authorization executor path mismatch")
    ...
    if (
        ...
        raise ValueError("formal-v5 readiness evidence does not authorize execution")
...
def prepare_authorized_run(
    *, authorization_path: Path, output_dir: Path
    ...
    if output_dir.exists():
        raise ValueError("formal-v5 output directory already exists")
    ...
    if dirty or head != upstream:
        raise ValueError("formal-v5 execution requires a clean pushed HEAD")
    ...
    if readiness.get("readiness_passed") is not True:
        raise ValueError("formal-v5 runtime readiness did not pass")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = implementation.execute_formal(
            config=config, manifest=manifest, readiness=readiness,
            output_dir=output_dir,
        )
    except (
        ...
        print(f"phase5_real_formal_execution_v5_error: {exc}", file=sys.stderr)
        return 2
