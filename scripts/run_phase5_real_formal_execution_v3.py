#!/usr/bin/env python3
"""Authorize and execute one fresh privacy-safe formal-v3 matrix."""
...
DEFAULT_AUTHORIZATION = (
    PROJECT_ROOT / "configs" / "phase5_real_formal_execution_v3.json"
)
AUTHORIZATION_VERSION = "phase5-real-thor-formal-execution-authorization-v3"
BASE_PROTOCOL_VERSION = "phase5-real-thor-pilot-v3"
READINESS_EVIDENCE_VERSION = "phase5-real-thor-formal-readiness-v3-result-v1"
AUTHORIZATION_EXECUTOR = "scripts/run_phase5_real_formal_execution_v3.py"


def _implementation() -> object:
    path = PROJECT_ROOT / "scripts" / "run_phase5_real_formal_pilot_v2.py"
    spec = importlib.util.spec_from_file_location("phase5_real_formal_v3_frozen", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load frozen formal-v3 executor")
...
def load_authorized_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("formal-v3 authorization must be a mapping")
    allowed = {
    ...
    if set(map(str, raw)) != allowed:
        raise ValueError("formal-v3 authorization has unexpected fields")
    if raw.get("authorization_version") != AUTHORIZATION_VERSION:
        raise ValueError("formal-v3 authorization version mismatch")
    if raw.get("formal_execution_authorized") is not True:
        raise ValueError("formal-v3 execution is not explicitly authorized")
    if raw.get("matrix_contract_override_allowed") is not False:
        raise ValueError("formal-v3 authorization cannot override the matrix")
    if raw.get("authorization_executor") != AUTHORIZATION_EXECUTOR:
        raise ValueError("formal-v3 authorization executor path mismatch")
    ...
    if (
        ...
        raise ValueError("formal-v3 readiness evidence does not authorize execution")
...
def prepare_authorized_run(
    *, authorization_path: Path, output_dir: Path
    ...
    if output_dir.exists():
        raise ValueError("formal-v3 output directory already exists")
    ...
    if dirty or head != upstream:
        raise ValueError("formal-v3 execution requires a clean pushed HEAD")
    ...
    if readiness.get("readiness_passed") is not True:
        raise ValueError("formal-v3 runtime readiness did not pass")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"phase5_real_formal_execution_v3_error: {exc}", file=sys.stderr)
        return 2
