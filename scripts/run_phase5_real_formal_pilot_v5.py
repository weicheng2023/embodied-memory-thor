#!/usr/bin/env python3
"""Build no-THOR readiness evidence for fresh real formal pilot v5."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "phase5_real_formal_pilot_v5.json"


def _implementation() -> object:
    path = PROJECT_ROOT / "scripts" / "run_phase5_real_formal_pilot_v2.py"
    spec = importlib.util.spec_from_file_location("phase5_real_formal_v5_shared", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load shared formal-v5 readiness executor")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"phase5_real_formal_v5_readiness_error: {exc}")
        return 2
